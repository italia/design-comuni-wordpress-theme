...
if($luogo->post_parent == 0){
	...
	if(!$indirizzo)	$indirizzo = dci_get_meta("indirizzo", "_dci_luogo_", $luogo->post_parent);
	if(!$posizione_gps["lat"] || !$posizione_gps["lng"]) $posizione_gps = dci_get_meta("posizione_gps", "_dci_luogo_", $luogo->post_parent);
	if(!$cap) $cap = dci_get_meta("cap", "_dci_luogo_", $luogo->post_parent);
	if(!$mail) $mail = dci_get_meta("mail", "_dci_luogo_", $luogo->post_parent);
	if(!$telefono) $telefono = dci_get_meta("telefono", "_dci_luogo_", $luogo->post_parent);
}
						...
						<div class="py-4">
							<ul class="location-list mt-2">
                                ...
                                <li><div class="location-title">
                                    ...
                                    <div class="location-content">
                                        <p>
                                            <a href="https://www.google.com/maps/dir/'<?php echo $posizione_gps["lat"]; ?>,<?php echo $posizione_gps["lng"]; ?>'/@<?php echo $posizione_gps["lat"]; ?>,<?php echo $posizione_gps["lng"]; ?>,15z?hl=it" target="_blank"><?php _e("Naviga su Google Map", "design_comuni_italia"); ?></a></p>
                                    </div>
                                </li>
							</ul><!-- /location-list -->
						</div>
