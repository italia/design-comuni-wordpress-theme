jQuery( document ).ready(function() {
    ...
    let inputDettaglioRichiesta = jQuery('textarea[name^="_dci_appuntamento_dettaglio_richiesta"]');
    inputDettaglioRichiesta.each(function() {
        jQuery(this).on('change keyup paste', function(){
            dci_remove_highlight_missing_field('.cmb2-id--dci-appuntamento-dettaglio-richiesta');
        });
    });
    ...
    jQuery( 'form[name="post"]' ).on('submit', function(e) {
        ...
        /**
         * controllo compilazione campo Dettaglio richiesta
         */
        if (!jQuery('textarea[name^="_dci_appuntamento_dettaglio_richiesta"]').val()) {
            dci_highlight_missing_field('.cmb2-id--dci-appuntamento-dettaglio-richiesta');
            return false;
        }
