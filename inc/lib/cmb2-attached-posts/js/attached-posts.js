...
(function (window, document, $, app, undefined) {
  ...
  app.init = function () {
    ...
    $('.cmb2-wrap > .cmb2-metabox')
      ...
      .on(
        'click',
        '.cmb-type-custom-attached-posts .cmb-td .cmb2-attached-posts-search-button',
        app._openSearch
      );
  ...
  app.resetAttachedListItems = function ($wrap) {
    ...
    $wrap.find('.attached li').each(function (index) {
      ...
      newVal.push(
        $(this)
          .attr('class', zebraClass + ' ui-sortable-handle')
          .data('id')
      );
    });
  ...
  app.replacePlusIcon = function () {
    $('.attached li .dashicons.dashicons-plus')
  ...
  app.rowTmpl = function (row) {
    ...
    return (
      ...
      '" class="' +
      row.class +
      ' ui-draggable ui-draggable-handle"><a title="' +
      app.editTitle +
      '" href="' +
      app.edit_link_template.replace('REPLACEME', row.id) +
      '">' +
      row.title +
      '</a>' +
      row.type +
      '<span class="dashicons dashicons-plus add-remove"></span></li>'
    );
  };
  ...
  app.SearchView = window.Backbone.View.extend({
    ...
    open: function () {
      ...
      this.$el
        .show()
        .find('#find-posts-head')
        .html(this.findtxt + '<div id="find-posts-close"></div>');
